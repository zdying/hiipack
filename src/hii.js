...
try{
    var _cache = path.resolve(__hiipack__.root, 'tmpl', '_cache');

    fse.copy(_cache, __hiipack__.tmpdir, function(err){
        if(err) console.error(err);
    });

    fse.copy(_cache, __hiipack__.tmpdirWithVersion, function(err){
        if(err) console.error(err);
    });
}catch(e){
